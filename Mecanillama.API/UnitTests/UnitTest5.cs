...
public class UnitTest5
{
    ...
    [TestMethod]
    public async Task UpdateMechanic_ReturnsNoContent()
    {
        // Crear un objeto MechanicRequest con los datos del mecánico a actualizar
        var mechanic = new Mechanic
        {
            Name = "Jhon",
            Address = "Calle Las Lomas 123",
            Description = "Hola me gusta arreglar",
            Phone = 999999999,
            UserId = 1
        };

        // Serializar el objeto MechanicRequest a formato JSON
        var jsonRequest = JsonConvert.SerializeObject(mechanic);

        // Crear un objeto HttpContent con el contenido JSON
        var content = new StringContent(jsonRequest, Encoding.UTF8, "application/json");

        // Realizar una solicitud HTTP PUT al endpoint de actualización de mecánicos
        HttpResponseMessage response = await client.PutAsync("/api/v1/mechanics/{id}", content);
